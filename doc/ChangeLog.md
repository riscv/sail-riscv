...
- Updates to the [configuration file](../config/config.json.in):
  ...
  - The `dirty_update` attribute of `memory.translation` has been removed,
    ...
    the PTE.
  - The global handling (i.e. before address translation) of misaligned
    accesses can be configured in a more fine-grained way: the handling of AMOs,
    LR/SC and scalar and vector load/stores can be individually specified.
    Misaligned AMOs and LR/SC now both raise access faults by default; this is a
    change from the previous default and now matches the more common behavior. See
    `memory.misaligned.global_faults`, which replaces `memory.misaligned.supported`.
  - The handling of misaligned accesses can also be similarly specified in a PMA.
    See the `attributes.misaligned_faults` field of each region under `memory.regions`;
    `attributes.misaligned_faults` replaces the earlier `attributes.misaligned_fault`
    property.
