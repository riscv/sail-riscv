...
#MARCH          	= " -march=rv64imc_zicsr -mabi=ilp64"
MARCH          	= " -march=rv64imc -mabi=lp64"
...
def test_dump__recipe(t) :
    echo("building " + t.target)
    cmd = OBJDUMP64 + " " + OBJDUMP_FLAGS + " " + ARTIFACT_DIR + "test.elf" + " > " + t.target
    echo("cmd: " + cmd)
...
def test_elf__recipe(t) :
    echo("building " + t.target)
    cmd = LD64 + LD_FLAGS + " -o " + t.target + " " + OBJS_BASE
    echo("cmd: " + cmd)
    return os.system(cmd)
...
def asm__recipe(t) :
    ...
    if (os.system("mkdir -p " + ARTIFACT_DIR) != 0) :
        ...
        sys.exit(1)
    cmd = ASM64 + " " + ASM_FLAGS + " -o " + t.target + " " + ' '.join(t.prerequisites)
    echo("cmd: " + cmd)
    return os.system(cmd)
