...
import lint_rvvi_trace
import rvviTextChecker
...
class LintModeAndAmoTests(unittest.TestCase):
    ...
    def _ret(self, insn_hex, extra="", mode="0x3"):
        return (
            ...
            + " "
            + extra
            + "MEM I 4 0x80000000 0x80000000 0 MODE "
            + mode
            + " VIRT 0x0\n"
        )
    ...
    def test_amo_wrong_width_is_caught(self):
        text = self._lines(
            self._ret(
                self.AMOADD_D,
                "X 7 0x0000000000000001 MEM D 4 0x80001000 0x80001000 0 ",
            )
        )
        errors = lint_rvvi_trace.lint_trace(text)
    ...
    def test_vector_load_per_element_mem_d_passes(self):
        ...
        text = self._lines(
            self._ret(self.VLE32_V, "V 1 0x0000000000000000 " + mems + " ")
        )
        self.assertEqual(lint_rvvi_trace.lint_trace(text), [])
