...
def check_STRING(token):
    if not token[0].isalpha():
        raise AssertionError(
            f"Token '{token}' is not a valid string (must start with an alphabetic character)."
        )
    for c in token:
        if c.isalnum() or c == "_" or c == " ":
            continue
        raise AssertionError(
            f"Token '{token}' is not a valid string (must contain only alphanumeric characters or underscores)."
        )
    return token
...
def check_PARAMS(state, tokens):
    ...
    if state.timescale not in timescales:
        raise AssertionError(
            f"TIMESCALE must be one of '{timescales}', got '{state.timescale}'."
        )
...
def check_HART(state, tokens):
    ...
    if state.hart >= state.nharts:
        raise AssertionError(
            f"HART ID {state.hart} exceeds maximum HARTS ({state.nharts})."
        )
...
def check_RET(state, tokens):
    ...
    if pc >= (1 << state.xlen):
        raise AssertionError(
            f"PC value {pc:#x} exceeds XLEN limit ({state.xlen} bits)."
        )

    inst_bin = check_HEX(tokens[2])
    if inst_bin >= (1 << state.ilen):
        raise AssertionError(
            f"Instruction binary value {inst_bin:#x} exceeds ILEN limit ({state.ilen} bits)."
        )
...
def check_X(state, tokens):
    ...
    if index >= 32:
        raise AssertionError(
            f"X register index {index} out of range (must be less than 32)."
        )

    value = check_HEX(tokens[2])
    if value >= (1 << state.xlen):
        raise AssertionError(
            f"X{index} register value {value:#x} exceeds XLEN limit ({state.xlen} bits)."
        )
    return tokens[3:]
...
def check_F(state, tokens):
    ...
    if index >= 32:
        raise AssertionError(
            f"F register index {index} out of range (must be less than 32)."
        )

    value = check_HEX(tokens[2])
    if value >= (1 << state.flen):
        raise AssertionError(
            f"F{index} register value {value:#x} exceeds FLEN limit ({state.flen} bits)."
        )
    return tokens[3:]
...
def check_V(state, tokens):
    ...
    if index >= 32:
        raise AssertionError(
            f"Vector register index {index} out of range (must be less than 32)."
        )

    value = check_HEX(tokens[2])
    if value >= (1 << state.vlen):
        raise AssertionError(
            f"V{index} register value {value:#x} exceeds VLEN limit ({state.vlen} bits)."
        )
    return tokens[3:]
...
def check_C(state, tokens):
    ...
    if index >= 0x1000:
        raise AssertionError(
            f"CSR index {index:#x} out of range (must be less than 0x1000)."
        )

    value = check_HEX(tokens[2])
    if value >= (1 << state.xlen):
        raise AssertionError(
            f"CSR {index:#x} register value {value:#x} exceeds XLEN limit ({state.xlen} bits)."
        )
...
def check_META(state, tokens):
    ...
    if len(tokens) < 2 + count:
        raise AssertionError(
            f"Not enough tokens for META data (expected {count} values)."
        )
...
def check_TIME(state, tokens):
    ...
    if time_delta < 0:
        raise AssertionError(
            f"TIME delta {time_delta} is invalid (must be a non-negative integer)."
        )
    return tokens[2:]
...
def check_CYCLE(state, tokens):
    ...
    if cycle_delta < 0:
        raise AssertionError(
            f"CYCLE delta {cycle_delta} is invalid (must be a non-negative integer)."
        )
    return tokens[2:]
...
def check_MEM(state, tokens):
    ...
    if bytes <= 0:
        raise AssertionError(
            f"Memory access byte count {bytes} is invalid (must be a positive integer)."
        )
    ...
    if count < 0:
        raise AssertionError(
            f"Memory access count {count} is invalid (must be a non-negative integer)."
        )
    ...
    for i in range(count):
        if len(tokens) < 2:
            raise AssertionError(
                "Not enough tokens for MEM data (expected key-value pairs)."
            )
        check_STRING(tokens[0])  # key
        key = tokens[0]
        value = tokens[1]
        # MEM values may be a name, hex, or int.
        if value.startswith(("0x", "0X")):
            ...
            try:
                ...
                check_STRING(value)
        if key in ("PT", "GPT") and value not in ("K", "M", "G", "T", "P"):
            raise AssertionError(
                f"MEM {key} value '{value}' is invalid (must be K, M, G, T, or P)."
            )
        tokens = tokens[2:]
...
def check_line(state, tokens):
    ...
    while tokens:
        ...
        if state.elm_index == 0 and key != "VERSION":
