...
def lint_trace(text: str) -> list[str]:
    ...
    for ev in events:
        ...
        if (
            ...
            and ev["mode"] is not None
            and ev["mode"] != prev_mode
            and prev_event != "TRAP"
            and prev_inst not in (MRET, SRET)
        ):
            errors.append(
        ...
        if ev["event"] == "RET" and not is_compressed(inst):
            ...
            if opcode in _RD_WRITE_OPCODES:
                if rd != 0 and rd not in x_indices:
                    errors.append(
                        f"{loc}: opcode {opcode:#09b} rd=x{rd} missing X {rd} record"
                    )
                if rd == 0 and x_indices:
                    errors.append(
                        f"{loc}: opcode {opcode:#09b} rd=x0 must not have X records, got {x_indices}"
                    )
            if opcode in _NO_RD_OPCODES and x_indices:
                errors.append(
