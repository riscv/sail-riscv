...
for test in $DIR/riscv-tests/Zilsd/rv32*.elf; do
    if timeout 5 $RISCVDIR/c_emulator/riscv_sim_RV32 -p --enable-zilsd  $test > ${test%.elf}.cout 2>&1 && grep -q SUCCESS ${test%.elf}.cout
    then
        green "C-32 $(basename $test)" "ok"
    else
        red "C-32 $(basename $test)" "fail"
...
for test in $DIR/riscv-tests/Zclsd/rv32*.elf; do
    if timeout 5 $RISCVDIR/c_emulator/riscv_sim_RV32 -p --enable-zilsd --enable-zclsd $test > ${test%.elf}.cout 2>&1 && grep -q SUCCESS ${test%.elf}.cout
    then
        green "C-32 $(basename $test)" "ok"
    else
        red "C-32 $(basename $test)" "fail"
