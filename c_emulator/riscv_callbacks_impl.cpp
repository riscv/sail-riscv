...
void callbacks_if_impl::mem_write_callback(const char *type, sbits paddr,
...
void callbacks_if_impl::mem_read_callback(const char *type, sbits paddr,
                                          uint64_t width, lbits value)
{
  if (config_print_mem_access) {
    fprintf(trace_log, "mem[%s,0x%0*" PRIX64 "] -> 0x", type,
...
void callbacks_if_impl::mem_exception_callback(sbits paddr,
...
void callbacks_if_impl::xreg_full_write_callback(const_sail_string abi_name,
...
void callbacks_if_impl::freg_write_callback(unsigned reg, sbits value)
{
...
void callbacks_if_impl::csr_full_write_callback(const_sail_string csr_name,
                                                unsigned reg, sbits value)
{
  if (config_print_reg) {
...
void callbacks_if_impl::csr_full_read_callback(const_sail_string csr_name,
                                               unsigned reg, sbits value)
{
  if (config_print_reg) {
    fprintf(trace_log, "CSR %s (0x%03X) -> 0x%0*" PRIX64 "\n", csr_name, reg,
...
void callbacks_if_impl::vreg_write_callback(unsigned reg, lbits value)
{
  if (config_print_reg) {
...
void callbacks_if_impl::pc_write_callback(sbits value)
{
  (void)value;
}

void callbacks_if_impl::trap_callback()
{
  if (config_enable_rvfi) {
    zrvfi_trap(UNIT);
  }
