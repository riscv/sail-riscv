...
uint64_t get_config_uint64(size_t key_elems, sail_config_key key)
{
  sail_config_json json = sail_config_get(key_elems, key);
  sail_int big_n;
  uint64_t n;

  if (!sail_config_is_int(json)) {
    fprintf(stderr, "Configuration option");
    exit(1);
  }
...
#define RST_VEC_SIZE 8
  ...
  sail_config_key rstvec_key = {"platform", "reset_vector"};
  uint64_t rom_base = get_config_uint64(2, rstvec_key);
  ...
  sail_config_key rom_base_key = {"platform", "rom", "base"};
  ...
  if (rom_size != get_config_uint64(3, rom_size_key)) {
    fprintf(stderr,
...
void run_sail(void)
{
  ...
  sail_config_key insns_per_tick_key = {"platform", "instructions_per_tick"};
  int insns_per_tick = (int)get_config_uint64(2, insns_per_tick_key);
