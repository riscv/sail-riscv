...
void
set_config_print(char *var, bool val)
{
    if (var == NULL || strcmp("all", var) == 0) {
        ...
        fprintf(stderr,
                "Unknown trace category: '%s' (should be "
                "instr|reg|mem|platform|all)\n",
                var);
        exit(1);
    }
}
...
static void
print_usage(const char *argv0, int ec)
{
#ifdef RVFI_DII
    fprintf(stdout,
...
void
check_elf(bool is32bit)
{
    if (is32bit) {
        if (zxlen_val != 32) {
            fprintf(stderr,
                    "32-bit ELF not supported by RV%" PRIu64 " model.\n",
                    zxlen_val);
            exit(1);
        }
    } else {
        if (zxlen_val != 64) {
            fprintf(stderr,
                    "64-bit ELF not supported by RV%" PRIu64 " model.\n",
                    zxlen_val);
            exit(1);
        }
    }
...
void
init_spike(const char *f, uint64_t entry, uint64_t ram_size)
{
#ifdef ENABLE_SPIKE
    bool mismatch = false;
    const char *isa = is_32bit_model() ? RV32ISA : RV64ISA;
    ...
    if (tv_is_misaligned_enabled(s) != rv_enable_misaligned) {
        mismatch = true;
        fprintf(stderr,
                "inconsistent enable-misaligned-access setting: spike %s, sail "
                "%s\n",
...
void
init_sail_reset_vector(uint64_t entry)
{
...
void
reinit_sail(uint64_t elf_entry)
{
    model_fini();
    model_init();
    init_sail(elf_entry);
...
int
init_check(struct tv_spike_t *s)
{
    int passed = 1;
#ifdef ENABLE_SPIKE
...
void
write_signature(const char *file)
{
    if (mem_sig_start >= mem_sig_end) {
        fprintf(stderr,
                "Invalid signature region [0x%0" PRIx64 ",0x%0" PRIx64
                "] to %s.\n",
...
int
compare_states(struct tv_spike_t *s)
{
    int passed = 1;
...
static void
get_and_send_rvfi_packet(packet_reader_fn *reader)
{
    lbits packet;
    CREATE(lbits)(&packet);
    reader(&packet, UNIT);
...
void
rvfi_send_trace(unsigned version)
{
    if (config_print_rvfi) {
...
run_sail(void)
{
    ...
    while (!zhtif_done && (insn_limit == 0 || total_insns < insn_limit)) {
        ...
        if (rvfi_dii) {
            ...
            if (res == -1) {
                fprintf(stderr, "Reading RVFI DII command failed: %s",
                        strerror(errno));
                exit(1);
            }
            if (res < sizeof(instr_bits)) {
                fprintf(stderr,
                        "Reading RVFI DII command failed: insufficient input");
                exit(1);
            }
            ...
            case 0: { /* EndOfTrace */
                ...
                if (insn == (('V' << 24) | ('E' << 16) | ('R' << 8) | 'S')) {
                    /*
                     * Reset with insn set to 'VERS' is a version negotiation
                     * request and not a actual reset request. Respond with a
                     * message say that we support version 2.
                     */
                    if (config_print_rvfi) {
                        fprintf(stderr, "EndOfTrace was actually a version "
                                        "negotiation packet.\n");
                    }
            ...
            case 'v': { /* Set wire format version */
                ...
                if (insn == 1) {
                    ...
                    fprintf(stderr,
                            "Requested trace in unsupported format %jd!\n",
                            (intmax_t)insn);
                    exit(1);
                }
                rvfi_trace_version = insn;    // From now on send traces in the
                                              // requested format
                struct
                {
                    char msg[8];
                ...
                if (write(rvfi_dii_sock, &version_response,
        ...
        if (zhtif_done) {
            if (!spike_done) {
                fprintf(stdout,
                        "Sail done (exit-code %" PRIi64 "), but not Spike!\n",
                        zhtif_exit_code);
                exit(1);
            }
        } else {
...
main(int argc, char **argv)
{
    ...
    if (rvfi_dii) {
        ...
        if (fd_flags == -1) {
            fprintf(stderr,
                    "Failed to get file descriptor flags for socket!\n");
            return 1;
        }
        if (config_print_rvfi) {
