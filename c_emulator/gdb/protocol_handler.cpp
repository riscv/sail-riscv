...
void protocol_handler::process_raw_request(std::string req) {
  ...
  if (resp.has_value()) {
    ...
    // TODO: send unsupported response.  But don't send any response
    // for now since the interaction stops without a response and this
    // helps to find protocol packets that may need to be supported.
  }
}
...
void protocol_handler::send_response(const std::string &resp) {
  ...
  // if any response sends binary data.  There is no such response as
  // yet.
  data.append(resp);
  data.append("#");
  {
...
void protocol_handler::mem_write_callback(ModelImpl &, const char *, sbits paddr, int64_t width, lbits) {
  // Do not reset `m_triggered` if it was already set.
...
// Another approach would be to intercept the `pc_write_callback` and put the check there.
