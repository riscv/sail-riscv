...
steady_clock::time_point run_end;
...
void finish() {
  ...
  if (do_show_times) {
    ...
    auto init_msecs = duration_cast<milliseconds>(init_end - init_start).count();
    auto exec_msecs = duration_cast<milliseconds>(run_end - init_end).count();
    uint64_t kips = total_insns / exec_msecs;
    fprintf(stderr, "Initialization:   %d ms\n", init_msecs);
    fprintf(stderr, "Execution:        %d ms\n", exec_msecs);
