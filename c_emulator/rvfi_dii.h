...
typedef struct {
  uint32_t rvfi_insn; // 31 ..  0, Instruction word: 32-bit instruction
                      // or command. The lower 16-bits may decode to a 16-bit
                      // compressed instruction.
  uint16_t rvfi_time; // 47 .. 32, Time to inject token. The difference between
  ...
  uint8_t rvfi_cmd;   // 55 .. 48, This token is a trace command. For example,
                      // reset device under test.
  uint8_t padding;    // 63 .. 56
...
typedef struct {
  uint64_t rvfi_order;     //  63 ..   0, [00 - 07] Instruction number: INSTRET
                           //  value after completion.
  uint64_t rvfi_pc_rdata;  // 127 ..  64, [08 - 15] PC before instr: PC for
                           // current instruction
  uint64_t rvfi_pc_wdata;  // 191 .. 128, [16 - 23] PC after instr: Following
                           // PC - either PC + 4 or jump/trap target.
  uint64_t rvfi_insn;      // 255 .. 192, [24 - 31] Instruction word: 32-bit
                           // command value.
  uint64_t rvfi_rs1_data;  // 319 .. 256, [32 - 39] Read register values:
                           // Values as read from registers named
  uint64_t rvfi_rs2_data;  // 383 .. 320, [40 - 47] above. Must be 0 if
                           // register ID is 0.          0 if unused.
  uint64_t rvfi_rd_wdata;  // 447 .. 384, [48 - 55] Write register value: MUST
                           // be 0 if rd_ is 0.
  uint64_t rvfi_mem_addr;  // 511 .. 448, [56 - 63] Memory access addr: Points
                           // to byte address (aligned if define
  uint64_t rvfi_mem_rdata; // 575 .. 512, [64 - 71] Read data: Data read from
                           // mem_addr (i.e. before write)
  uint64_t rvfi_mem_wdata; // 639 .. 576, [72 - 79] Write data: Data written
                           // to memory by this command.
  uint8_t rvfi_mem_rmask;  // 647 .. 640, [80] Read mask: Indicates valid
                           // bytes read. 0 if unused.
  uint8_t rvfi_mem_wmask;  // 655 .. 648, [81] Write mask: Indicates
                           // valid bytes written. 0 if unused.
  uint8_t rvfi_rs1_addr;   // 663 .. 656, [82] Read register addresses:
                           // Can be arbitrary when not used,
  uint8_t rvfi_rs2_addr;   // 671 .. 664, [83] otherwise set as decoded.
  uint8_t rvfi_rd_addr;    // 679 .. 672, [84] Write register address: MUST
                           // be 0 if not used.
  uint8_t rvfi_trap;       // 687 .. 680, [85] Trap indicator: Invalid
                           // decode, misaligned access or  misaligned address.
  uint8_t rvfi_halt;       // 695 .. 688, [86] Halt indicator: Marks the
                           // last instruction retired before halting execution.
  uint8_t rvfi_intr;       // 703 .. 696, [87] Trap handler: Set for
...
typedef struct /* bits(512) */ {
  ...
  uint64_t trace_size; // 127 .. 64, total size of the trace packet + extensions
  RVFI_DII_Execution_Packet_InstMetadata basic_data; // 319 .. 128
  RVFI_DII_Execution_Packet_PC pc_data;              // 447 .. 320
  uint64_t available_fields;                         // 511 .. 448,
...
RVFI_DII_Instruction_Packet
RVFI_DII_Instruction_Packet_from_u64(uint64_t value);
