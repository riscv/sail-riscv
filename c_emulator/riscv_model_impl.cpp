...
unit ModelImpl::vmem_access_callback(sbits vaddr, sbits paddr, bool is_fetch, bool is_write, int64_t width) {
  for (auto c : m_callbacks) {
    c->vmem_access_callback(*this, vaddr, paddr, is_fetch, is_write, width);
  }
  return UNIT;
}
